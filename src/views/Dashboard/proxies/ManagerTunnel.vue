...
const splitDomain = (domain: string) => {
  ...
  if (parts.length <= 2) return { host: '@', rootDomain: domain }
...
const rules: FormRules = {
  proxyName: { required: true, message: '请输入隧道名称', trigger: ['blur', 'input'] },
  localIp: { required: true, message: '请输入本地地址', trigger: ['blur', 'input'] },
  localPort: {
    required: true, type: 'number', message: '请输入本地端口', trigger: ['blur', 'input'],
    validator: (_rule: any, value: number) => (value < 1 || value > 65535) ? new Error('端口范围必须在 1-65535 之间') : true
  },
  remotePort: {
    required: true, message: '请输入远程端口', trigger: ['blur', 'input'],
    validator: (_rule: any, value: number) => {
      if (editForm.value.proxyType === 'http' || editForm.value.proxyType === 'https') return true
      return (value < 1 || value > 65535) ? new Error('端口范围必须在 1-65535 之间') : true
    }
  },
  domain: {
    validator: () => {
      if (editForm.value.proxyType === 'http' || editForm.value.proxyType === 'https') {
        if (!domainTags.value.length) return new Error('请至少添加一个域名')
      }
      return true
    },
...
const toggleModalTitle = computed(() => proxyToOperate.value ? (proxyToOperate.value.isDisabled ? '启用确认' : '禁用确认') : '')
const toggleModalContent = computed(() => proxyToOperate.value ? (proxyToOperate.value.isDisabled ? '确认要启用此隧道吗？' : '确认要禁用此隧道吗？') : '')
...
const getNodeStatus = (nodeId: number) => {
  const n = nodeList.value.find((n: any) => n.nodeId === nodeId)
  return n ? n.status : false
}
    ...
    userApi.get("/proxy/node/list", accessHandle(), (data) => {
      if (data.code === 0) {
        nodeOptions.value = (data.data || []).map((node: any) => ({
          label: node.name, value: node.nodeId, hostname: node.hostname
        }))
...
const fetchProxies = async () => {
  ...
  try {
    ...
    }, (msg) => message.error(msg || '获取隧道列表失败'), (err) => message.error(err.message || '获取隧道列表失败'))
  } catch (e: any) {
    message.error(e?.response?.data?.message || '获取隧道列表失败')
...
// -------------------- 事件处理 --------------------
const handleRefresh = () => { fetchProxies() }
const handleSelect = (key: string, proxy: Proxy) => {
  switch (key) {
    case 'view': selectedProxy.value = proxy; showModal.value = true; break
    case 'edit': handleEdit(proxy); break
    case 'toggle': proxyToOperate.value = proxy; showToggleModal.value = true; break
    case 'delete': proxyToDelete.value = proxy; showDeleteModal.value = true; break
  }
}
const handleEdit = (proxy: Proxy) => {
  ...
  try { domainTags.value = proxy.domain ? JSON.parse(proxy.domain) : [] } catch { domainTags.value = proxy.domain ? [proxy.domain] : [] }
  ...
  editFormRef.value?.validate(async (errors) => {
    if (!errors) {
      ...
      try {
        ...
        }, (msg) => message.error("更新隧道失败:" + msg || '更新隧道失败'), () => { loading.value = false })
      } catch (e: any) {
...
const handleDeleteConfirm = () => {
  ...
  loading.value = true
  try {
    userApi.post("/proxy/delete", { proxyId: proxyToDelete.value.proxyId }, accessHandle(), (data) => {
      if (data.code === 0) {
    ...
    }, (msg) => message.error(msg || '删除隧道失败'))
    showDeleteModal.value = false
  } catch (e: any) {
    message.error(e?.response?.data?.message || '删除隧道失败')
  } finally { loading.value = false }
}
const handleToggleConfirm = async () => {
  ...
  try {
    ...
    }, (msg) => message.error(msg || '操作失败'))
  } catch (e) { message.error(e) }
  finally { loading.value = false; showToggleModal.value = false }
}
const handleGetFreePortForEdit = async () => {
  ...
  try {
    userApi.post("/proxy/freePort", {
      ...
      else message.error(data.message || '获取空闲端口失败')
    })
  } catch (e: any) {
    message.error(e?.response?.data?.message || '获取空闲端口失败')
  } finally { gettingFreePort.value = false }
}
...
const renderDomainTag = (tag: string) => {
  return h(NTag, {
    round: true, closable: true, style: 'cursor: pointer',
    onClose: () => {
      const idx = domainTags.value.indexOf(tag)
      if (idx !== -1) {
...
const dropdownOptions = (proxy: Proxy) => [
  { label: '查看详情', key: 'view', icon: renderIcon(InformationCircleOutline) },
  { type: 'divider', key: 'd1' },
  { label: '编辑', key: 'edit', icon: renderIcon(CreateOutline) },
  { type: 'divider', key: 'd2' },
  { label: proxy.isDisabled ? '启用' : '禁用', key: 'toggle', icon: renderIcon(PowerOutline) },
...
const handleStarProxy = async (proxy: Proxy) => {
  ...
  if (proxy.isDisabled) return message.error('此隧道已被禁用，请先启用后再操作')
  loading.value = true
  try {
    if (!proxy.isOnline) {
      const success = await invoke('start_proxy', { proxyId: proxy.proxyId, token: token.value })
...
const columns = [
  {
    title: 'ID', key: 'proxyId',
    render(row: Proxy) { return h(NTag, { type: 'info', size: 'medium', round: true }, { default: () => `# ${row.proxyId}` }) }
  },
  {
    title: '名称', key: 'proxyName',
    render(row: Proxy) { return h(NText, { style: 'white-space: nowrap; overflow: hidden; text-overflow: ellipsis;' }, { default: () => row.proxyName }) }
  },
  {
    title: '类型', key: 'proxyType',
    render(row: Proxy) { return h(NTag, { size: 'small', round: true }, { default: () => row.proxyType.toUpperCase() }) }
  },
  {
    title: '远程端口', key: 'remotePort',
    render(row: Proxy) { return h(NText, { code: true }, { default: () => row.remotePort }) }
  },
  {
    title: '节点', key: 'nodeId',
    render(row: Proxy) { return h(NText, { depth: 3, style: 'white-space: nowrap; overflow: hidden; text-overflow: ellipsis;' }, { default: () => getNodeLabel(row.nodeId) }) }
  },
  {
    title: '状态', key: 'status',
    render(row: Proxy) {
      return h(NSpace, { size: 4 }, {
        default: () => [
          h(NTag, { type: row.isOnline ? 'success' : 'error', size: 'small', round: true }, { default: () => row.isOnline ? '在线' : '离线' }),
          row.isBanned && h(NTag, { type: 'error', size: 'small', round: true }, { default: () => '已封禁' }),
          row.isDisabled && h(NTag, { type: 'warning', size: 'small', round: true }, { default: () => '已禁用' })
        ].filter(Boolean)
      })
    }
  },
    ...
    render(row: Proxy) {
      return h(NDropdown, {
        ...
        default: () => h(NButton, {
          ...
          icon: () => h(NIcon, null, { default: () => h(EllipsisHorizontalCircleOutline) })
        })
      })
    }
  }
...
const checkFrpcHas = async () => {
  ...
  } catch (e) { console.error('检查frpc.exe失败:', e) }
}
</script>
