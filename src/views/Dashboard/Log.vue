...
function updateDisplayedLogs() {
  ...
  // 获取当前选择的隧道日志哈希列表
  const hashes = selectedTunnel.value === 'all' 
    ? Array.from(logStore.value.keys()) // 显示所有日志
    : Array.from(tunnelLogIndices.value[selectedTunnel.value] || new Set());
  ...
  .filter((log): log is string => log !== undefined)
    .sort((a, b) => {
      ...
      const timeB = b.match(/\[(\d{1,2}:\d{1,2}:\d{1,2})\]/)?.[1] || '';
...
function saveLogsToStorage() {
  try {
    ...
    localStorage.setItem('frpcLogs', allLogs.join('\n'));
    console.log(`保存了 ${allLogs.length} 条日志到localStorage`);
  } catch (e) {
...
function pruneLogsIfNeeded() {
  ...
  if (logCount > MAX_LOG_COUNT * 1.5) { // 提高触发清理的阈值
    console.log(`日志数量(${logCount})超过上限，进行清理...`);
    
    // 仅清理最旧的10%日志
    const allHashes = Array.from(logStore.value.keys());
    const hashesToRemove = allHashes.slice(0, Math.floor(allHashes.length * 0.1));
    
    for (const hash of hashesToRemove) {
      logStore.value.delete(hash);
      for (const tunnelId of Object.keys(tunnelLogIndices.value)) {
        tunnelLogIndices.value[tunnelId]?.delete(hash);
      }
    }
...
onMounted(async () => {
  ...
  if (savedLogs) {
    ...
    for (const line of logLines) {
      if (line.trim()) {
        const hash = hashCode(normalizeLogContent(line));
        if (!logStore.value.has(hash)) {
          logStore.value.set(hash, line);
        }
      }
